...
class Actor {
    ...
    isIntersect(actor) {
    	if (isActor(actor)) {
        	...
        	if ((this.right <= actor.left)||(actor.right <= this.left)||(this.bottom <= actor.top)||(actor.bottom <= this.top)) return false
        	return true
		} else throw(new Error)	
    }
}
...
class Level {
    ...
    obstacleAt(newPos, size) {
        for(let xCoord = newPos.x; xCoord <= newPos.x + size.x; xCoord++) {
            for(let yCoord = newPos.y; yCoord <= newPos.y + size.y; yCoord++) {
                ...
                if (this.grid[Math.floor(yCoord)][Math.floor(xCoord)]) {
                    return this.grid[Math.floor(yCoord)][Math.floor(xCoord)]
                }
            }
        }
        return undefined
...
class LevelParser {
    ...
    createActors(stringMap) {
        let yCoord = 0
        let resultArray = []
        if (!this.glossary) return resultArray
        for(let string of stringMap) {
    ...
    parse(stringMap) {
        let grid = this.createGrid(stringMap)
        let actors = this.createActors(stringMap)
...
class Fireball extends Actor {
    ...
    act(time, level) {
    	if (level.obstacleAt(this.getNextPosition(time), this.size)) {
            this.handleObstacle()
        } else this.pos = this.getNextPosition(time)
    }
}
